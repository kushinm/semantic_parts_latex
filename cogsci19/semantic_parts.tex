...
\textsuperscript{1}Department of Cognitive Science, Vassar College (\texttt{kumukherjee@vassar.edu})\\
\textsuperscript{2}Department of Psychology, Stanford University (\texttt{\{rxdh,jefan\}@stanford.edu})\\ 
\textsuperscript{3}Department of Psychology, University of California, San Diego (\texttt{jefan@ucsd.edu})}
...
\begin{abstract}
\jefan{Between the title and author list would be a great place to place a 2-column spanning banner containing several example annotated sketches. Getting this idea from the way SIGGRAPH conference papers are usualyl formatted, would be a great way immediately signal to the reader the kind of data we're working with.}
...
\caption{Dog}
\end{subfigure}
\caption{Average number of strokes used to draw each part per category. While participants use multiple strokes to represent some parts, other parts are sometimes expressed using single strokes. It is possible that}

...
\author{
\textbf{Kushin Mukherjee}  \\ Department of Cognitive Science \\  Vassar College \\  \texttt{kumukherjee@vassar.edu}
 \And
\textbf{Robert X. D. Hawkins} \\ Department of Psychology \\  Stanford University \\ \texttt{rxdh@stanford.edu}
 \And
...
\renewcommand{\@maketitle}{\@oldmaketitle% Update \@maketitle to insert...
 ...
 \captionof{figure}{\footnotesize{Objects used in communication game with example drawings below, where stroke color indicates different parts.}}\bigskip}
...
To investigate this, we collected data on the part information expressed in drawings by having participants densely annotate drawings of real-world objects.
Our dataset contained both detailed and sparser drawings produced in different communicative contexts.
We found that: (1) people are consistent in what they interpret individual strokes to represent; (2) single strokes tend to correspond to single parts, with strokes representing the same part often being clustered in time; and (3) both sparse and detailed drawings of the same object emphasize similar part information, although detailed drawings of different objects are more distinct from one another than sparse drawings. 
...
\textbf{Keywords:} 
compositionality; objects and categories; perceptual organization; sketch understanding; visual communication 
\end{abstract}
...
However, the question of how semantic information \textit{within} drawings is organized has not been investigated as thoroughly (cf. \citeNP{li2018fast,schneider2016example}).
...
% \kushin{The viewer was allowed to make a guess only after the sketcher had finished drawing, so there was no imposed time-constraint that may have driven a minimalistic approach to production.}
This context manipulation led sketchers to produce relatively simpler drawings containing fewer strokes and less ink on far trials than on close trials, while still achieving high recognition accuracy in both contexts.

% (Fig.~\ref{refgame_performance}C). %, Fig.~\ref{refgame_gallery}B\&C). 
...
Prior works analyzing the semantic properties of drawing data have used a raster image representation (e.g., \texttt{*.png}), an expedient format for applying modern convolutional neural network architectures \shortcite{FanCommon2018,sangkloy2016sketchy,yu2017sketch}. 
However, to investigate how semantic structure manifests during drawing production, it was critical to encode each drawing using a vector image format that preserves the inherently sequential and contour-based nature of drawing production (e.g., \texttt{*.svg}). 
Thus, each drawing in our dataset is represented as a sequence of individual strokes. 
A stroke is defined as the mark left by a virtual pen on a digital drawing canvas between being `placed onto' the canvas and being `lifted up'. %, with a commonly used interface involving clicking and dragging a mouse cursor, followefd by release of the mouse button. 
We parameterized each stroke by a sequence of cubic Bézier curves, called \textit{splines}.
This format provides a compact representation of drawing data, which also preserves the sequence in which each element was produced.

\subsection{Semantic part annotation}

We crowdsourced dense semantic annotations for every spline in every stroke of the drawings from this dataset. 
...
If any spline was too short for annotators to feasibly annotate it with their mouse cursor, it was concatenated with its neighboring splines until the resulting spline was long enough to easily select. 
...
% Note: Labels for 8261 out of 83474 splines were remapped.
% This resulted in us refining the total number of unique labels in our dataset from 304 to 24. 

We first standardized all 304 distinct labels provided by participants, mapping them to a common set of 24 part labels that applied to all objects in the dataset.
...
We only examined drawings that were annotated by at least three distinct participants, providing a consistent way to evaluate annotation consistency across splines. 
To reduce bias due to missing data, we also restricted our analyses to annotation trials in which the drawing was completely annotated (i.e., all splines were tagged). 
After applying all preprocessing, our resulting dataset consisted of 864 drawings that had been completely annotated 3 times.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.44\textwidth]{figures/5_stroke_part_relationship.pdf}
...
Indeed, strokes spanning multiple parts were slightly more common in drawings produced in far contexts (19.4\%, CI: [17.9\%, 20.9\%]) than close contexts (17.6\%, CI: [16.1\%, 18.8\%]\footnote{95\% confidence intervals were estimated via stratified bootstrap resampling (N=1000 iterations) of drawings within each context condition.}, \textit{p}$ = 0.07$), suggesting that sketchers were somewhat more likely to use a single stroke to represent multiple contiguous parts in a context where a sparser drawing would be sufficient.
...
% In other words, how does the temporal sequence of stroke production relate to semantic part structure? %% I generally like to avoid having to use the construction: "In other words" -- if we have to say this, then we didn't say it clearly enough the first time... 
...
% \kushin{METACOMMENT: Should we leave the CIs for the z-scores as they are or should we just report mean z-score?}
%% good question: We might be okay saying that the mean we're reporting is the z-score _across drawings_, and the CI is for the mean z-score...
...
% (2) To the extent close and far drawings of the same object emphasize part information differently, does this reflect large differences in emphasis on a few parts, or small differences in emphasis across many parts? %% really liked your edit here, way more streamlined. 
...
% In order to represent all drawings in our dataset using a common feature representation, we combined part labels across categories, yielding a set of 24 unique part labels to which any stroke could be assigned. %% we already explained that collapsed to 24 labels during preprocessing
% \kushin{Although some part-labels appeared in multiple object-categories (e.g., leg appeared in dog, bird, and chair), we chose to use common vector dimensions for the same part across categories. 
% Since all our analyses were within-category there was no need to create category-specific part-labels for these shared labels.}
%% I think going out of our way to say this would potentially cause more confusion than necessary... especially since we already explained earlier that we are using the "common set" of 24 part labels. 
...
Consistent with this, we found strong correlations between the feature vectors for close and far drawings of the same object ($r = 0.73$,  CI: [0.68, 0.77])\footnote{95\% confidence intervals were estimated via stratified bootstrap resampling (N=10000 iterations) of drawings within each object-context combination.}, which were significantly stronger than close and far drawings of \emph{different} objects ($r = 0.64$, CI: [0.60,  0.68]; same objects vs. different objects:  \textit{p}$ < 0.001$).
...
It would thus be valuable to apply up our analytical approach to the large drawing datasets \cite{eitz2012humans,sangkloy2016sketchy,quickdraw2017} that have provided the basis for these modeling approaches.

In ongoing work, we are extending our analysis of how different part information is expressed in drawings beyond simple effort cost measures (i.e., number of strokes, amount of ink) to encompass content and style information (e.g., the shape of a bird's wing, caricaturization of a chair's armrest). 
...
KM was supported by the Department of Cognitive Science at Vassar College through its Humanities in Cognitive Science program and the Center for the Study of Language and Information at Stanford University.
...
\href{https://github.com/cogtoolslab/semantic_parts}{\url{https://github.com/cogtoolslab/semantic_parts}}
}}}
\vspace{3em} \noindent 

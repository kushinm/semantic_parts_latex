...
A natural possibility is that the actions they perform are guided by knowledge of that object's parts. 
...
Concretely, we hypothesized that most strokes in our dataset would not cross part boundaries: that all splines within a given stroke would be assigned the same part label. 
...
Conversely, because depictions of parts can be arbitrarily detailed, and some parts re-occur throughout an object (e.g., multiple legs on a bird, chair, or dog), it is natural for there to be more than one stroke per part.

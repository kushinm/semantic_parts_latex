...
This approach departs from the conventional strategy for inferring the organization of visual object concepts, which typically relies on eliciting judgments about visual inputs, usually with respect to experimenter-defined dimensions. 
By contrast, visual communication tasks permit participants to include any elements they consider relevant and combine these elements freely in production, yielding high-dimensional information about how visual semantic knowledge is organized and deployed under a natural task objective. 
...
\textit{first}, a focus on their compositional part structure using a new dataset of dense part annotations,
\textit{second}, exploration of the link between this structure and the temporal dynamics of drawing production,
...
This data format provides a compact representation of each drawing compared with a raster image, and preserves the sequence in which each element was produced. 
...
However, participants could generate their own part label if they believed none of the common labels applied.
...
\subsection{How well do people agree on what strokes mean?}
...
This shows that the way people interpret what part each stroke represents is highly systematic, providing critical validation for our general approach. 
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.48\textwidth]{figures/6_part_sequence.pdf}

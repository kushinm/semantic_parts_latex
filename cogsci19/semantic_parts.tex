...
Indeed, strokes spanning multiple parts were slightly more common in drawings produced in far contexts (19.4\%, CI: [17.9\%, 20.9\%]) than close contexts (17.6\%, CI: [16.1\%, 18.8\%]\footnote{95\% confidence intervals were estimated via stratified bootstrap resampling (N=1000 iterations) of drawings within each object-context combination.}), suggesting that sketchers were somewhat more likely to use a single stroke to represent multiple contiguous parts in a context where a sparse drawing would be sufficient.
%This suggests that sketchers 
And the proportion of parts requiring more than one stroke was slightly higher for close drawings (55.8\%, CI: [53.7\%, 58.6\%]) than far drawings (52.0\%, CI: [49.9\%, 54.6\%]), suggesting that they may have included more detail per part to distinguish the target object from similar distractors. 
% >>>>>>> 6ddb6c5c6cccd2b66c57cafa376616dbccfdad72
...
% The above findings show that close and far drawings exhibited similar patterns of emphasis on different parts. 
...
How are these additional strokes being spent? 

% \jefan{I think we should report the basic observation that close drawings consist of more strokes and higher arclengths than far, with CIs, around here.}

We hypothesized that the additional part information provided in close sketches makes close drawings more distinguishable from one another, relative to far ones, by increasing the feature distance between objects that otherwise share many perceptual properties. 

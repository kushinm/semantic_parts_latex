...
%% Key aspect of human visual perception and cognition is ability to extract semantically meaningful, structured information from raw sensory inputs. Drawings are natural and effective way of exposing this. 

%% Classical approaches to understanding real-world perceptual organization posited geometric primitives (i.e., geons), exhibiting strong inductive bias towards hierarchical structure in objects. 

%% Modern approaches to modeling visual perception employ end-to-end training on large image datasets, avoiding explicitly `hand-engineered' structured representations. The resulting representations have been pretty useful for making progress on general-purpose image understanding, including sketch understanding. 

%% However, capturing image-level correspondences between sketches and images still requires substantial supervision, and there is not a clear way of encoding one of the major axes of variation across sketches -- visual abstraction. 

%% Our study is motivated by classical ideas from object perception and recent currents in machine learning that focus on learning high-level structured representations of objects. 

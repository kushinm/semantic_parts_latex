...
Here we consider the notion that the compositional nature of object concepts enable us to readily decompose both objects and drawings of objects into a common set of semantically meaningful parts.
...
We first obtained 1195 drawings of 32 real-world objects from a recent experimental dataset in which pairs of participants played an interactive drawing-based reference game \shortcite{fan2018modeling}.\footnote{All materials and data will be made available following the completion of the review process.}
...
Thus, each drawing in our dataset is represented as a sequence of individual strokes, where each stroke consists of a sequence of sub-stroke elements, parameterized as cubic Bezier splines.
This format provides a compact representation that preserves the sequence in which each element was produced.
%of each drawing compared with a raster image, and preserves the sequence in which each element was produced. 
...
Thus, most strokes represented exactly one part, but there are still a minority of cases where a single stroke spanned multiple parts (e.g., a single stroke connecting the head and body of a bird, or an armrest and leg of a chair). 

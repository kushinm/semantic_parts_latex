...
\caption{(A) Object renderings. (B) Example sketches produced in a close context. (C) Example sketches produced in a far context. (D-E) Sketchers used fewer strokes in the far condition, while producing sketches that were accurately recognized by the viewer in both conditions.}
\label{sketch_gallery}
\end{figure*}

We obtained 1198 drawings of 32 real-world objects from a recent experimental dataset in which participants were paired in an online environment to play a sketching-based reference game \cite{fan2018modeling}.
...
Objects belonged to one of four basic-level categories (i.e., bird, car, chair, dog), containing eight exemplars each.  
One participant (i.e., the sketcher) aimed to draw one of these objects --- the target --- so that the other participant (i.e., the viewer) could pick it out from a set of distractor objects. 
...
This context manipulation led sketchers to produce simpler sketches containing fewer strokes and less ink on far context trials than on close trials, while still achieving high recognition accuracy in both contexts. 

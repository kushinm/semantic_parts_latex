...
If any given spline was too short for annotators to feasibly annotate them with their mouse cursor, it was concatenated with its neighboring splines until the resulting spline was long enough to easily select. 
...
\textit{First,} we remapped spline labels to common labels and only after that did we subset for sketches having 3 complete annotations.
This \kushin{first} preprocessing step was performed in order to incorporate manual part labels provided by participants (i.e. labels not in the menu).

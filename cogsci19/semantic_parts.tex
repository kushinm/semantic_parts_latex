...
Given the communicative nature of the reference game that generated this dataset, we believe drawers would make strokes that were easily interpretable. We take the fact that the majority of naive annotators seemed to consistently agree on the semantic content of these strokes to be a good estimation of what the drawers wished to convey through their actions.
...
% \jefan{I recommend using the same language from the abstract where we enumerate the main findings, preserving the sequence in which those findings appear in the paper. The wording can be slightly different but it should be immediately recognizable as parallel to the earlier summary of findings.}

%%%%%%%%%%%%%%%%% informal sketch of discussion %%%%%%%%%%%%%%%%%%%%%%%

%% summary of main findings parallel to abstract

%% what is distinct about the current approach is its focus on linking fine-grained semantic knowledge about real objects to dynamics of drawing production. this is distinct from lake omniglot paper, which did not consider real objects. this is distinct from most approaches to studying fine-grained semantic knowledge, b/c of focus on production. 

%% the stakes: the importance of considering compositionality in human cognition has manifested in many domains, including language, perception, causal reasoning, etc. Compositionality is cool b/c it naturally supports flexibility and generalization. Current work shows that communicative drawings display compositionality. This is cool b/c compositionality-in-drawings becomes then an important target for modern theories/models of human visual cognition, which should be able to display this kind of human-like robustness, flexibility, expressivity. 

%% future work should use data like ours as benchmarks to validate models of visual production (e.g., sketch-rnn and that new contour drawing paper: https://arxiv.org/pdf/1901.00542.pdf) -- do they emphasize part information in a similar way? do they accomplish shifts in abstraction in a similar way to the way humans do?
...
% \jefan{Good start here ... I'd suggest having two paragraphs here: (1) The importance of considering compositionality in human cognition manifests in many domains, including language, perception, causal reasoning, etc. Compositionality is cool b/c it naturally supports flexibility and generalization. Current work shows that communicative drawings display compositionality. This is cool b/c this is then an important target for modern theories/models of human visual cognition, which should also display this kind of robustness and flexibility. (2) }

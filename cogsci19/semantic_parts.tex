...
\subsection{How are parts emphasized in different communicative contexts?}

% Our findings so far bear how the way people compose communicative sketches of objects reflects their semantic knowledge of the parts that objects are composed from. 
...
% For example, when trying to communicate about a bird in a far context containing only objects from different basic-level categories, sketchers 

% spontaneously used fewer strokes while achieving high accuracy in communicating the identity of the target. 

% By the same token, when trying to communicate about that same bird (e.g., a nightingale) in a close context containing other, perceptually similar birds, sketchers used more strokes in order to distinguish the target bird from the others. 

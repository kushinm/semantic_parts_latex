...
\caption{(A) Visualization of the distribution of mean feature vectors for each object-condition combination, projected onto the top two principal components. (B) Feature vectors of close and far sketches of the same object were more similar to each other than those of close and far sketches of different objects within a category. (C) Far sketches were more similar to each other than close sketches were to each other. *** indicates \textit{p}$<0.001.$}
\label{part_emphasis}
\end{figure}
...
The above findings show that close sketches exhibited similar patterns of emphasis on different parts as their far counterparts. 
However, given that close sketches contain greater emphasis on these parts overall than far sketches (i.e., contain more and longer strokes), a natural question affect how this influences how discriminable they are.
\jefan{Kushin, I think we should report the basic observation that close sketches consist of more strokes and higher arclengths than far, with CIs, around here.}
We hypothesized this overall greater part information enhances the discriminability of close sketches from one another, relative to far ones, by increasing the feature distance between objects that otherwise share many perceptual properties. 
...
When the sequential strokes that a drawer makes to produce a drawing are coded in terms of these parts we see that there is a prevalent one-to-one correspondence between individual strokes and parts. 
...
The structure of the completed drawing reflects this semantic part knowledge, even when sketches are produced in different contexts to different degrees of detail. Detailed drawings of objects distinguish themselves from their sparser counterparts by preferentially expressing object-specific parts through their constituent strokes.

By focusing on strokes and their sequence, we are able to study the concrete decisions people take in order to produce drawings terms of their semantic content. 
And these reveal to us that people are able to decompose an abstract object into part-categories and reconstruct it in a structured manner during graphical production. 
Through sketches, people reveal their compositional knowledge of objects and the relationship between these constituent parts,
...
%The prevalent 
 correspondences between strokes made by the sketcher and part-labels assigned to those strokes by annotators indicate that this abstract knowledge is deployed for both graphical production and recognition.
